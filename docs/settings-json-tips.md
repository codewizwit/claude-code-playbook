...
Claude Code's `settings.json` controls permissions, tool access, and hooks. This page explains each setting in the [recommended starter](../.claude/settings.json) and how to customize them.
...
The starters all include "never read `.env` files" as a written rule. The settings.json makes it enforceable:
...
The [recommended starter](../.claude/settings.json) includes a **docs-check hook** — a `PostToolUse` hook on Bash that fires after every shell command. When it detects a `git commit` that changed code but no documentation, it tells Claude to check if docs need updating.
